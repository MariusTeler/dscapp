...
class LocalitatiService
{
    ...
    public static function getByLocalitateJudet(string $judet, string $localitate): array
    {
        if(empty($localitate) || empty($judet)) {
            return [];
        }
    ...
    public static function getKmExteriori(int $localitate_id): int
    {
        if($localitate_id <= 0) return 0;
        $row = DB::table('localitati')
            ->select('dist_km')
            ->where('cod_lc', $localitate_id)
            ->first();
        return $row ? intval($row->dist_km) : 0;
    }
}
