...
class AwbPdfService extends TCPDF
{
	...
	public function makeHalfFirstPage(int $xi, int $yi): void
	{
	  //MultiCell( $w, $h, $txt, $border = 0, $align = 'J', $fill = false, $ln = 1, $x = '', $y = '', $reseth = true, $stretch = 0, $ishtml = false, $autopadding = true, $maxh = 0, $valign = 'T', $fitcell = false )
		...
		$this->MultiCell(10, 10, (isset($this->item['destinatar_centru_rut_bvh']) ? $this->item['destinatar_centru_rut_bvh'] : "0"), 1, 'C', true, 0, $x + 29, '', true, 0, false, true, 10, 'M');
		...
		$this->setFont('helvetica','B', 22, '');
		$this->MultiCell(10, 10, (isset($this->item['destinatar_centru_rut_buh']) ? $this->item['destinatar_centru_rut_buh'] : "0"), 1, 'C', true, 0, $x + 92, '', true, 0, false, true, 10, 'M');
		$this->setFont('helvetica','B', 16, '');
		...
		$saveY = $this->GetY();
		//error_log("SAVEY: ".$saveY." : {$this->item['destinatar_centru_rut_buc']}");
		if(!empty($this->item['destinatar_centru_rut_buc'])){
			$this->setFont('helvetica','B', 28, '');
			$this->SetTextColor(255,255,255);
			$this->SetFillColor(0, 0, 0);

			$this->MultiCell(58, 10, '', 0, '', 0, 0, $x + 127, $saveY - 10, true, 0, false, true, 10, 'M', false);
			$this->MultiCell(11, 10, $this->item['destinatar_centru_rut_buc'], 1, 'C', 1, 1, $this->GetX(), $saveY - 10, true, 0, false, true, 10, 'M', true);
	...
	public function makePuisorMultiCell(string $code, int $nr_colet, int $position = 0): void
	{
		...
		$this->item['valoare_totala_cu_tva_text'] = "(cu TVA): --- " .$this->item['moneda'];
        if($this->item['mod_plata'] == 0){
			$this->item['transport_text'] .= "la " . ($this->item['platitor_id'] == $this->item['expeditor_id'] ? "PRELUARE" : "LIVRARE");
			$valoare_totala_cu_tva = number_format(round(floatval($this->item['valoare_fara_tva']) + floatval($this->item['valoare_tva']), 2), 2, '.', '');
			$this->item['valoare_totala_cu_tva_text'] = "(cu TVA): {$valoare_totala_cu_tva} {$this->item['moneda']}";
        }
		...
		$this->setFont('helvetica','B', 28, '');
		$this->MultiCell(10, 10, (isset($this->item['destinatar_centru_rut_bvh']) ? $this->item['destinatar_centru_rut_bvh'] : "0"), 1, 'C', 1, 0, $x + 24, '', true, 3, false, true, 10, 'M', true);
		...
		$this->setFont('helvetica','B', 28, '');
		$this->MultiCell(10, 10, (isset($this->item['destinatar_centru_rut_buh']) ? $this->item['destinatar_centru_rut_buh'] : "0"), 1, 'C', 1, 1, $x + 85, '', true, 3, false, true, 10, 'M', true);
		...
		$this->MultiCell(40, 5, "{$this->item['valoare_totala_cu_tva_text']}", 0, 'C', 0, 1, $x + 55, '', true, 3, false, true, 5, 'M', true);
		...
		$this->setFont('helvetica','B', 28, '');
		$this->MultiCell(10, 10, !empty($this->item['destinatar_centru_rut_buc']) ? $this->item['destinatar_centru_rut_buc'] : "", 1, 'C', 1, 1, $x + 85, '', true, 3, false, true, 10, 'M', true);
