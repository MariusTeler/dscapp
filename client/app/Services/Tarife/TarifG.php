...
class TarifG { // tarif de lista -> id_cl=0 in tabla tarife
	...
	{
        try {
		    return $tipObj == self::PALET ? self::selectPalet($tarifeDetId, $tipTarif, $km) :  self::selectColet($tarifeDetId, $tipTarif);
        } catch (\Exception $e) {
    ...
    {
        try {
            Log::debug("Calcul valoare greutate : tarifeG = " . json_encode($tarifeG) . " , greutate = {$greutate} , greutate_vol = {$greutate_vol} , indexC = {$indexC}");
            if($tarifeG->isEmpty()) $tarifeG = self::selectColet();
            $greutate = round(floatval($greutate), 2);
            $greutate_vol = round(floatval($greutate_vol), 2);
            Log::debug("Calcul valoare greutate : tarifeG = " . json_encode($tarifeG) . " , greutate = {$greutate} , greutate_vol = {$greutate_vol} , indexC = {$indexC}");
        
            if(!empty($greutate_vol) && $greutate < $greutate_vol) $greutate = $greutate_vol;
            ...
            while($greutate > 0 && !$stop && $i < 100)
            {
                ...
                $tarifeG->each(function($row) use (&$greutate, &$val, &$stop, &$val_g_fin)
                {
                    if(!$stop)
                    {	
                        if(round(floatval($row->inc_greut), 2) == round(floatval($row->g_fin), 2)) //palier
                            $val = floatval($row->inc_val) + floatval($row->val_init);
                        else if(round(floatval($row->inc_greut), 2) == round(floatval($row->g_fin) - floatval($row->g_init), 2)) //paliere
                            $val += floatval($row->inc_val) + floatval($row->val_init);
                        else if(round(floatval($row->inc_greut), 2) < round(floatval($row->g_fin), 2)) //per kg
                        {
                            $temp = 0.00;
                            if($greutate > round(floatval($row->g_fin), 2)) { 
                                $temp = round(floatval($row->g_fin) - floatval($row->g_init), 2); 
                            } else { 
                                $temp = round($greutate - floatval($row->g_init), 2); 
                                $stop = true;
                            }
                    ...
                    if($greutate > round(floatval($row->g_init), 2) && $greutate <= round(floatval($row->g_fin), 2)) // am gasit greutatea ... ma opresc : stop
                        $stop = true;
                    $val_g_fin = round(floatval($row->g_fin), 2);
                });
                $greutate -= $val_g_fin;
                $ret_val += $val;
                $i++;
            ...
            Log::debug("Calcul valoare greutate : final ret_val = {$ret_val}");
